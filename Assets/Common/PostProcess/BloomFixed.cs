...
namespace Overheat.Common.PostProcessing
{
	...
	/// </summary>
	[Serializable]
	[PostProcess(typeof(BloomRenderer), PostProcessEvent.AfterStack, "Custom/BloomFixed")]
	public sealed class BloomFixed : PostProcessEffectSettings
	{
		/// <summary>
	...
	internal sealed class BloomRenderer : PostProcessEffectRenderer<BloomFixed>
	{
		private static readonly int AutoExposureTex = Shader.PropertyToID("_AutoExposureTex");
		//private static readonly int HistogramBuffer = Shader.PropertyToID("_HistogramBuffer");
		private static readonly int Params = Shader.PropertyToID("_Params");
		//private static readonly int ScaleOffsetRes = Shader.PropertyToID("_ScaleOffsetRes");
		...
		private static readonly int Bloom_DirtTex = Shader.PropertyToID("_Bloom_DirtTex");
		private static readonly int Bloom_Settings = Shader.PropertyToID("_Bloom_Settings");
		private static readonly int Bloom_Color = Shader.PropertyToID("_Bloom_Color");
		private static readonly int Bloom_DirtTileOffset = Shader.PropertyToID("_Bloom_DirtTileOffset");

		private static readonly FieldInfo fieldUberSheet = typeof(PostProcessRenderContext).GetField("uberSheet", BindingFlags.NonPublic | BindingFlags.Instance);
		private static readonly FieldInfo fieldBloomBufferNameID = typeof(PostProcessRenderContext).GetField("bloomBufferNameID", BindingFlags.NonPublic | BindingFlags.Instance);
		private static readonly FieldInfo fieldAutoExposureTexture = typeof(PostProcessRenderContext).GetField("autoExposureTexture", BindingFlags.NonPublic | BindingFlags.Instance);
		...
		public override void Render(PostProcessRenderContext context)
		{
			...
			sheet.properties.SetTexture(AutoExposureTex, (Texture)fieldAutoExposureTexture.GetValue(context));
			...
			// Bla-bla-bla. Halfres introduces insane amount of flickering.
			...
			sheet.properties.SetFloat(SampleScale, sampleScale);
			...
			sheet.properties.SetVector(Threshold, threshold);
			float lclamp = Mathf.GammaToLinearSpace(settings.clamp.value);
			sheet.properties.SetVector(Params, new Vector4(lclamp, 0f, 0f, 0f));
			...
			for (int i = iterations - 2; i >= 0; i--) {
				int mipDown = m_Pyramid[i].down;
				int mipUp = m_Pyramid[i].up;
				cmd.SetGlobalTexture(BloomTex, mipDown);
			...
			if (context.IsDebugOverlayEnabled(DebugOverlay.BloomThreshold)) {
				...
				sheet.properties.SetVector(ColorIntensity, new Vector4(linearColor.r, linearColor.g, linearColor.b, intensity));
			...
			float screenRatio = context.screenWidth / (float)context.screenHeight;
			...
			// Shader properties
			var uberSheet = (PropertySheet)fieldUberSheet.GetValue(context);
			if (settings.fastMode)
				uberSheet.EnableKeyword("BLOOM_LOW");
			else
				uberSheet.EnableKeyword("BLOOM");

			uberSheet.properties.SetVector(Bloom_DirtTileOffset, dirtTileOffset);
			uberSheet.properties.SetVector(Bloom_Settings, shaderSettings);
			uberSheet.properties.SetColor(Bloom_Color, linearColor);
			uberSheet.properties.SetTexture(Bloom_DirtTex, dirtTexture);
			cmd.SetGlobalTexture(BloomTex, lastUp);
			...
			fieldBloomBufferNameID.SetValue(context, lastUp);
		}
	}
}
*/
